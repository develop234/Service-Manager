...
import users
user = users.User()
user.create("john@TESTSARP.com", "password")
user.update("john@TESTSARP.com", password="new password", role=users.Role.admin)
print(user.role("john@TESTSARP.com"))
> "admin"
...
class User(object):
    ...
    def role(self, email):
        ...
        Example:
        > user = User()
        > print(user.role("test@TESTSARP.com")==user.Role.admin)
        """
...
def test_user():
    ...
    user = User()
    assert user.create("test@TESTSARP.com", "test")==1
    assert user.role("test@TESTSARP.com")==Role.user
    assert user.authenticate("test@TESTSARP.com", "test") is True
    assert user.canAnnotate("test@TESTSARP.com") is False
    assert user.update("test@TESTSARP.com", "", "annotator")==1
    assert user.canAnnotate("test@TESTSARP.com") is True
    assert user.update("test@TESTSARP.com", "new password", "annotator")==1
    assert user.authenticate("test@TESTSARP.com", "test") is False
    assert user.authenticate("test@TESTSARP.com", "new password") is True
    assert user.delete("test@TESTSARP.com")==1
